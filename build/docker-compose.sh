...
NODE4="${1}_chain30_1"
#CLI4="docker exec ${NODE4} /root/bnchain-cli"

NODE5="${1}_chain29_1"
CLI5="docker exec ${NODE5} /root/bnchain-cli"
...
function base_init() {
    ...
    sed -i $sedfix 's/^Title.*/Title="local"/g' bnchain.toml
    sed -i $sedfix 's/^TestNet=.*/TestNet=true/g' bnchain.toml

    sed -i $sedfix 's/^powLimitBits=.*/powLimitBits="0x1f2fffff"/g' bnchain.toml
    sed -i $sedfix 's/^targetTimePerBlock=.*/targetTimePerBlock=1/g' bnchain.toml

    # p2p
    sed -i $sedfix 's/^seeds=.*/seeds=["bnchain:13802","chain32:13802","chain31:13802"]/g' bnchain.toml
    #sed -i $sedfix 's/^enable=.*/enable=true/g' bnchain.toml
    sed -i $sedfix '0,/^enable=.*/s//enable=true/' bnchain.toml
    sed -i $sedfix 's/^isSeed=.*/isSeed=true/g' bnchain.toml
    sed -i $sedfix 's/^innerSeedEnable=.*/innerSeedEnable=false/g' bnchain.toml
    sed -i $sedfix 's/^useGithub=.*/useGithub=false/g' bnchain.toml

    # rpc
    sed -i $sedfix 's/^jrpcBindAddr=.*/jrpcBindAddr="0.0.0.0:8801"/g' bnchain.toml
    sed -i $sedfix 's/^grpcBindAddr=.*/grpcBindAddr="0.0.0.0:8802"/g' bnchain.toml
    sed -i $sedfix 's/^whitelist=.*/whitelist=["localhost","127.0.0.1","0.0.0.0"]/g' bnchain.toml

    # wallet
    sed -i $sedfix 's/^minerdisable=.*/minerdisable=false/g' bnchain.toml

    sed -i $sedfix 's/^nodeGroupFrozenCoins=.*/nodeGroupFrozenCoins=20/g' bnchain.toml
    sed -i $sedfix 's/^paraConsensusStopBlocks=.*/paraConsensusStopBlocks=100/g' bnchain.toml

    # ticket
    sed -i $sedfix 's/^ticketPrice =.*/ticketPrice = 10000/g' bnchain.toml
...
function check_docker_status() {
    status=$(docker-compose ps | grep bnchain_1 | awk '{print $6}')
    statusPara=$(docker-compose ps | grep bnchain_1 | awk '{print $3}')
    if [ "${status}" == "Exit" ] || [ "${statusPara}" == "Exit" ]; then
        echo "=========== bnchain service Exit logs ========== "
        docker-compose logs bnchain
        echo "=========== bnchain service Exit logs End========== "
    fi

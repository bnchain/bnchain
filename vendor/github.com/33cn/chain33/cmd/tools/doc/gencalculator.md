...
### 代码生成
##### 生成基本代码
>使用bnchain-tool，工具使用参考[文档](https://github.com/bnchain/bnchain/blob/master/cmd/tools/doc/gendapp.md)
```
//本例默认将calculator生成至官方plugin项目dapp目录下
$ bnchain-tool gendapp -n calculator -p calculator.proto
$ cd $GOPATH/src/github.com/bnchain/plugin/plugin/dapp/calculator
//显示生成目录结构
$ tree -d
.
├── cmd //官方ci目录
...
分别是bnchain.CreateTransaction和bnchain.Query，上述代码只是示例如何开发rpc接口，
实际开发中，这两类接口可以不用实现，
...
import (
	...
	ptypes "github.com/bnchain/plugin/plugin/dapp/calculator/types/calculator"
)
```
...
func createAdd(cmd *cobra.Command, args []string) {
	...
	bnchainReq := rpctypes.CreateTxIn{
	...
	var res string
	//通过框架rpc调用
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.CreateTransaction", bnchainReq, &res)
	//通过合约内部实现rpc调用
...
```go
 ...
 func queryCalcCount(cmd *cobra.Command, args []string) {
 	...
 	bnchainReq := &rpctypes.Query4Jrpc{
 		Execer:   ptypes.CalculatorX,
 		FuncName: "CalcCount",
 		Payload:  types.MustPBToJSON(&req),
 	}
 	...
 	//调用框架Query rpc接口
 	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.Query", bnchainReq, &res)
 	//调用合约内部rpc接口
...
```go
 import (
 	_ "github.com/bnchain/plugin/plugin/dapp/calculator" //auto gen
 ```

##### 编译
>直接通过官方makefile文件
```
$ cd $GOPATH/src/github.com/bnchain/plugin && make
```

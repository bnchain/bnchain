...
func (up *updateInitStrategy) Run() error {
	mlog.Info("Begin run bnchain update init.go.")
	defer mlog.Info("Run bnchain update init.go finish.")
...
func (up *updateInitStrategy) initMember() error {
	...
	if err != nil || path == "" {
		if len(gopath) > 0 {
			path = filepath.Join(gopath, "/src/github.com/bnchain/bnchain/plugin/")
		}
	}
	if packname == "" {
	...
	if len(path) == 0 {
		return errors.New("bnchain Plugin Not Existed")
	}

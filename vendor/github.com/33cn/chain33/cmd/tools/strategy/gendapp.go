...
import (
	...
	"github.com/bnchain/bnchain/cmd/tools/tasks"
	"github.com/bnchain/bnchain/cmd/tools/types"
	"github.com/bnchain/bnchain/cmd/tools/util"
)
...
func (ad *genDappStrategy) Run() error {
	fmt.Println("Begin generate bnchain dapp code.")
	defer fmt.Println("End generate bnchain dapp code.")
	if !ad.initMember() {
...
func (ad *genDappStrategy) initMember() bool {
	...
	if outDir == "" {
		outDir = filepath.Join("github.com", "bnchain", "plugin", "plugin", "dapp")
	}
	//兼容win 反斜杠路径

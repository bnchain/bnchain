...
func (c *createPluginStrategy) Run() error {
	fmt.Println("Begin run bnchain create plugin project mode.")
	defer fmt.Println("Run bnchain create plugin project mode finish.")
...
func (c *createPluginStrategy) initMember() error {
	...
	c.gopath = gopath
	c.outRootPath = filepath.Join(gopath, "/src/github.com/bnchain")

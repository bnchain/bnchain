...
//RunbnchainCli invoke bnchain client
func RunbnchainCli(para []string) (string, error) {
...
func SendTxCommand(cmd string) (string, bool) {
	...
	output, err := RunbnchainCli(strings.Fields(cmd))
	if err != nil {
...
func GetTxInfo(txHash string) (string, bool) {
	...
	bReady := false
	txInfo, err := RunbnchainCli(strings.Fields(fmt.Sprintf("tx query -s %s", txHash)))
...
func CalcUtxoAvailAmount(addr string, txHash string) (float64, error) {
	...
	outStr, err := RunbnchainCli(strings.Fields(fmt.Sprintf("privacy showpai -d 1 -a %s", addr)))
...
func CalcUtxoSpendAmount(addr string, txHash string) (float64, error) {
	...
	outStr, err := RunbnchainCli(strings.Fields(fmt.Sprintf("privacy showpas -a %s", addr)))

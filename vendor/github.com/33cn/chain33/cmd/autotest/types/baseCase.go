...
import (
	...
	bnchainType "github.com/bnchain/bnchain/system/dapp/commands/types"
)
...
//check item handler
//适配autotest早期版本，handlerfunc的参数为json的map形式，后续统一使用bnchain的TxDetailResult结构体结构
...
type CheckHandlerParamType *bnchainType.TxDetailResult
...
func (pack *BaseCasePack) CheckResult(handlerMap interface{}) (bCheck bool, bSuccess bool) {
	...
	if bReady {
		...
		var txRecp bnchainType.TxDetailResult
		pack.TxReceipt = txInfo

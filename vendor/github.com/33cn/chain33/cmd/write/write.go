...
import (
	...
	coinstypes "github.com/bnchain/bnchain/system/dapp/coins/types"
...
func scanWrite() {
	for {
		...
		err = rpc.Call("bnchain.GetTxByAddr", paramsReqAddr, &replyTxInfos)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			continue
		}
		...
		for _, hash := range txHashes {
			...
			err = rpc.Call("bnchain.QueryTransaction", paramsQuery, &transactionDetail)
			if err != nil {
				fmt.Fprintln(os.Stderr, err)
				continue
			}
			...
			var signed string
			rpc.Call("bnchain.SignRawTx", paramsReqSignRawTx, &signed)
			paramsRaw := rpctypes.RawParm{
				Data: signed,
			}
			var sent string
			rpc.Call("bnchain.SendTransaction", paramsRaw, &sent)
			f, err := os.OpenFile(heightFile, os.O_RDWR, 0666)
			if err != nil {
				fmt.Fprintln(os.Stderr, err)
				continue

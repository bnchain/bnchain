...
import (
	...
	qmocks "github.com/bnchain/bnchain/queue/mocks"
...
func TestJSONClient_Call(t *testing.T) {
	...
	var result = ""
	err = jsonClient.Call("bnchain.Version", nil, &result)
	assert.NotNil(t, err)
	...
	err = jsonClient.Call("bnchain.Version", nil, &nodeVersion)
	assert.Nil(t, err)
	assert.Equal(t, "6.0.2", nodeVersion.bnchain)

	var isSnyc bool
	err = jsonClient.Call("bnchain.IsSync", &types.ReqNil{}, &isSnyc)
	assert.Nil(t, err)
...
func testCreateTxCoins(t *testing.T, jsonClient *jsonclient.JSONClient) {
	...
	var res string
	err := jsonClient.Call("bnchain.CreateRawTransaction", req, &res)
	assert.Nil(t, err)
...
func TestGrpc_Call(t *testing.T) {
	...
	client := types.NewbnchainClient(c)

...
import (
	...
	_ "github.com/bnchain/bnchain/rpc/grpcclient" // register grpc multiple resolver
...
// bnchain  a channel client
type bnchain struct {
	cli channelClient
	//for communicate with main chain in parallel chain
	mainGrpcCli types.bnchainClient
}
...
func NewJSONRPCServer(c queue.Client, api client.QueueProtocolAPI) *JSONRPCServer {
	...
	j.s = server
	err := server.RegisterName("bnchain", j.jrpc)
	if err != nil {
		return nil
	}
	return j

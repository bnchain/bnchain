...
function config_bnchain() {
    ...
    #update fee
    #    sed -i $sedfix 's/Fee=.*/Fee=100000/' ${bnchainConfig}

    #update wallet store driver
    #    sed -i $sedfix '/^\[wallet\]/,/^\[wallet./ s/^driver.*/driver="leveldb"/' ${bnchainConfig}
}
...
function start_bnchain() {
    ...
    echo "# start solo bnchain"
    rm -rf ../local/datadir ../local/logs ../local/grpc33.log
    ./bnchain -f bnchain.test.toml >/dev/null 2>&1 &
    local SLEEP=1
    sleep ${SLEEP}

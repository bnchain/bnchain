...
// package cli Runbnchain函数会加载各个模块，组合成区块链程序
//主循环由消息队列驱动。
//消息队列本身可插拔，可以支持各种队列
//同时共识模式也是可以插拔的。
//rpc 服务也是可以插拔的
...
import (
	...
	clog "github.com/bnchain/bnchain/common/log"
...
var (
	...
	datadir    = flag.String("datadir", "", "data dir of bnchain, include logs and datas")
...
//Runbnchain : run bnchain
func Runbnchain(name string) {
	flag.Parse()
	if *versionCmd {
	...
	log.Info(cfg.Title + "-app:" + version.GetAppVersion() + " bnchain:" + version.GetVersion() + " localdb:" + version.GetLocalDBVersion() + " statedb:" + version.GetStoreDBVersion())

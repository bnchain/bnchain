...
import (
	...
	_ "github.com/bnchain/bnchain/system/consensus/init"
...
func initEnv3() (queue.Queue, queue.Module, queue.Module, *Mempool) {
	...
	cfg, sub := types.InitCfg("../../cmd/bnchain/bnchain.test.toml")
...
func initEnv2(size int) (queue.Queue, *Mempool) {
	var q = queue.New("channel")
	cfg, _ := types.InitCfg("../../cmd/bnchain/bnchain.test.toml")
	types.Init(cfg.Title, cfg)
	blockchainProcess(q)
	execProcess(q)
...
func initEnv4(size int) (queue.Queue, *Mempool) {
	...
	var q = queue.New("channel")
	cfg, _ := types.InitCfg("testdata/bnchain.test.toml")

...
func peerInfo(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.GetPeerInfo", nil, &res)
	ctx.Run()
}
...
func isClockSync(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	var res bool
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.IsNtpClockSync", nil, &res)
	ctx.Run()
}
...
func isSync(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	var res bool
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.IsSync", nil, &res)
	ctx.Run()
}
...
func netInfo(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.GetNetInfo", nil, &res)
	ctx.Run()
}
...
func fatalFailure(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	var res int64
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.GetFatalFailure", nil, &res)
	ctx.Run()
}
...
func timestatus(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.GetTimeStatus", nil, &res)
	ctx.Run()
}

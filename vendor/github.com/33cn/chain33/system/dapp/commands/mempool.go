...
import (
	...
	"github.com/bnchain/bnchain/system/dapp/commands/types"
...
func listMempoolTxs(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.GetMempool", nil, &res)
...
func lastMempoolTxs(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	var res rpctypes.ReplyTxList
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.GetLastMemPool", nil, &res)
...
func properFee(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.GetProperFee", nil, &res)
	ctx.SetResultCb(nil)
	ctx.Run()
}

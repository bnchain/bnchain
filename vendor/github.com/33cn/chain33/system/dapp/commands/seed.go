...
func genSeed(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.GenSeed", params, &res)
	_, err := ctx.RunResult()
	if err != nil {
		fmt.Println(err)
		return
...
func getSeed(cmd *cobra.Command, args []string) {
	...
	var res types.ReplySeed
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.GetSeed", params, &res)
	ctx.Run()
}
...
func saveSeed(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.SaveSeed", params, &res)
	ctx.Run()
	//钱包解锁60s
	...
	jsonclient.NewRPCCtx(rpcLaddr, "bnchain.UnLock", params1, &res1)
}

...
func totalCoins(cmd *cobra.Command, args []string) {
	...
	if height == -1 {
		...
		err = rpc.Call("bnchain.GetLastHeader", nil, &res)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			return
		}
	...
	err = rpc.Call("bnchain.GetBlocks", params, &res)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		return
	}
	...
	if symbol == "bty" {
		...
		err = rpc.Call("bnchain.GetBlockHash", params, &res1)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			return
		}
		...
		err = rpc.Call("bnchain.QueryTotalFee", params2, &res2)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			return
		}
		...
		err = rpc.Call("bnchain.Query", params, &res)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			return
		}
	...
	if actual != "" {
		...
		for count = 100; count == 100; {
			...
			err = rpc.Call("bnchain.GetTotalCoins", params, &res)
			if err != nil {
				fmt.Fprintln(os.Stderr, err)
				return
			}
...
func execBalance(cmd *cobra.Command, args []string) {
	...
	for {
		...
		var str string
		err = rpc.Call("bnchain.GetExecBalance", reqParam, &str)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			return
		}

...
func SetTestNetFork() {
	systemFork.SetFork("bnchain", "ForkChainParamV1", 110000)
	systemFork.SetFork("bnchain", "ForkChainParamV2", 1692674)
	systemFork.SetFork("bnchain", "ForkCheckTxDup", 75260)
	systemFork.SetFork("bnchain", "ForkBlockHash", 209186)
	systemFork.SetFork("bnchain", "ForkMinerTime", 350000)
	systemFork.SetFork("bnchain", "ForkTransferExec", 408400)
	systemFork.SetFork("bnchain", "ForkExecKey", 408400)
	systemFork.SetFork("bnchain", "ForkWithdraw", 480000)
	systemFork.SetFork("bnchain", "ForkTxGroup", 408400)
	systemFork.SetFork("bnchain", "ForkResetTx0", 453400)
	systemFork.SetFork("bnchain", "ForkExecRollback", 706531)
	systemFork.SetFork("bnchain", "ForkTxHeight", 806578)
	systemFork.SetFork("bnchain", "ForkCheckBlockTime", 1200000)
	systemFork.SetFork("bnchain", "ForkMultiSignAddress", 1298600)
	systemFork.SetFork("bnchain", "ForkStateDBSet", 1572391)
	systemFork.SetFork("bnchain", "ForkBlockCheck", 1560000)
	systemFork.SetFork("bnchain", "ForkLocalDBAccess", 1572391)
	systemFork.SetFork("bnchain", "ForkTxGroupPara", 1687250)
	systemFork.SetFork("bnchain", "ForkBase58AddressCheck", 1800000)
	//这个fork只影响平行链，注册类似user.p.x.exec的driver，新开的平行链设为0即可，老的平行链要设置新的高度
	systemFork.SetFork("bnchain", "ForkEnableParaRegExec", 0)

}

func setLocalFork() {
	err := systemFork.CloneZero("bnchain", "local")
	if err != nil {
		panic(err)
	}
...
func setForkForParaZero(paraName string) {
	err := systemFork.CloneZero("bnchain", paraName)
	if err != nil {
...
func RegisterDappFork(dapp, fork string, height int64) {
	systemFork.SetDappFork("bnchain", dapp, fork, height)
}
...
func HasFork(fork string) bool {
	return systemFork.HasFork("bnchain", fork)
}
...
func initForkConfig(title string, forks *ForkList) {
	if title == "bnchain" { //bnchain fork is default set in code
		return
	}
	println(title)
	bnchainfork := systemFork.GetAll("bnchain")
	if bnchainfork == nil {
		panic("bnchain fork not init")
	}
	//开始判断bnchainfork中的system部分是否已经设置
	s := ""
	for k := range bnchainfork {

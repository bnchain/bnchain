...
import (
	...
	"github.com/bnchain/plugin/plugin/dapp/evm/commands/compiler"
	...
	"github.com/bnchain/bnchain/common/crypto/sha3"
	...
	common2 "github.com/bnchain/plugin/plugin/dapp/evm/executor/vm/common"
	evmtypes "github.com/bnchain/plugin/plugin/dapp/evm/types"
...
// transfer address format between ethereum and bnchain
...
func createEvmTx(action proto.Message, execer, caller, addr, expire, rpcLaddr string, fee uint64) (string, error) {
	...
	err = client.Call("bnchain.SignRawTx", unsignedTx, &res)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		return "", err
	}
...
func sendQuery(rpcAddr, funcName string, request types.Message, result proto.Message) bool {
	...
	err = jsonrpc.Call("bnchain.Query", params, result)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		return false
	}

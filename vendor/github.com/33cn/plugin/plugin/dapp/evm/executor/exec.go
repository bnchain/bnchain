...
import (
	...
	"github.com/bnchain/plugin/plugin/dapp/evm/executor/abi"
	"github.com/bnchain/plugin/plugin/dapp/evm/executor/vm/common"
	"github.com/bnchain/plugin/plugin/dapp/evm/executor/vm/model"
	"github.com/bnchain/plugin/plugin/dapp/evm/executor/vm/runtime"
	"github.com/bnchain/plugin/plugin/dapp/evm/executor/vm/state"
...
func (evm *EVMExecutor) GetMessage(tx *types.Transaction) (msg *common.Message, err error) {
	...
	// 此处暂时不考虑消息发送签名的处理，bnchain在mempool中对签名做了检查
	from := getCaller(tx)
	to := getReceiver(tx)
	if to == nil {

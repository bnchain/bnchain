...
// Client33 to connect with bnchain
...
func NewClient33(cfg *bnchain) *Client33 {
	conn, err := grpc.Dial(cfg.Host, grpc.WithInsecure())
	if err != nil {
		panic(err)
	}

	client := types.NewbnchainClient(conn)
	c := &Client33{
		config:        cfg,
		closer:        conn,
		bnchainClient: client,
	}
	return c
}
...
out:
	for {
		...
		case <-time.After(time.Second * 60):
			err := c.ping(ctx)
			if err != nil {
				log.Error("heartbeat", "heartbeat bnchain error: ", err.Error(), "reconnectAttempts: ", reconnectAttempts)
				c.autoReconnect(ctx)
				reconnectAttempts--
			} else {
				reconnectAttempts = c.config.ReconnectAttempts
...
// Start begin heartbeat to bnchain
...
func (c *Client33) autoReconnect(ctx context.Context) {
	if c.isClosed && !c.config.DisableAutoReconnect {
		...
		client := types.NewbnchainClient(conn)
		c.closer = conn
		c.bnchainClient = client
		c.isClosed = true
		c.Start(ctx)
	}
}

// SendTransaction send tx to bnchain
func (c *Client33) SendTransaction(ctx context.Context, in *types.Transaction) (*types.Reply, error) {
	...
	return c.bnchainClient.SendTransaction(ctx, in)
}

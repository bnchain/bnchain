...
import (
	...
	ty "github.com/bnchain/plugin/plugin/dapp/relay/types"
...
func NewRelayd(config *Config) *Relayd {
	...
	client33 := NewClient33(&config.bnchain)
	var btc BtcClient
...
func (r *Relayd) querybnchainWithBtcHeight() (*ty.ReplayRelayQryBTCHeadHeight, error) {
...
func (r *Relayd) syncBlockHeaders() {
	...
	if knownBtcHeight > r.firstHeaderHeight {
		ret, err := r.querybnchainWithBtcHeight()
		if err != nil {
			log.Error("syncBlockHeaders", "querybnchainWithBtcHeight error: ", err)
			return
		}

		log.Info("syncBlockHeaders", "querybnchainWithBtcHeight result: ", ret)

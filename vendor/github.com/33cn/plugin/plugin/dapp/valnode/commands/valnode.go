...
import (
	...
	ttypes "github.com/bnchain/plugin/plugin/consensus/tendermint/types"
	vt "github.com/bnchain/plugin/plugin/dapp/valnode/types"
...
func getBlockInfo(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.Query", params, &res)
	ctx.Run()
}
...
func createFiles(cmd *cobra.Command, args []string) {
	...
	genDoc := ttypes.GenesisDoc{
		ChainID:     fmt.Sprintf("bnchain-%v", RandStr(6)),

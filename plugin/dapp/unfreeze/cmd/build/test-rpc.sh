...
function query_unfreezeID() {
    ...
    local times=10
    while true; do
        req='{"method":"bnchain.QueryTransaction","params":[{"hash":"'"$txhash"'"}]}'
...
function init() {
    ...
    bnchain_SendToAddress "$owner" "$exec_addr" 500000000 "${MAIN_HTTP}"
    bnchain_SendToAddress "$beneficiary" "$exec_addr" 500000000 "${MAIN_HTTP}"
    block_wait 1
}
...
function CreateRawUnfreezeCreate() {
    ...
    bnchain_SignRawTx "$rawtx" "$owner_key" "${MAIN_HTTP}"
    query_unfreezeID
}

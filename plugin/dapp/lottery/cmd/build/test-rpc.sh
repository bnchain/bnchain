...
init() {
    ...
    if [[ $ispara == true ]]; then
        lottExecAddr=$(curl -ksd '{"method":"bnchain.ConvertExectoAddr","params":[{"execname":"user.p.para.lottery"}]}' ${MAIN_HTTP} | jq -r ".result")
        bnchain_SignRawTx "${lottery_addCreator_unsignedTx_para}" "${lottery_creator_priv}" ${MAIN_HTTP}
    else
        lottExecAddr=$(curl -ksd '{"method":"bnchain.ConvertExectoAddr","params":[{"execname":"lottery"}]}' ${MAIN_HTTP} | jq -r ".result")
        bnchain_SignRawTx "${lottery_addCreator_unsignedTx}" "${lottery_creator_priv}" ${MAIN_HTTP}
    fi
...
lottery_LotteryCreate() {
    ...
    priv=$1
    set -x
    resp=$(curl -ksd '{"method":"bnchain.CreateTransaction","params":[{"execer":"lottery","actionName":"LotteryCreate",
    ...
    bnchain_SignRawTx "${rawTx}" "${priv}" ${MAIN_HTTP}
    set +x
...
lottery_LotteryBuy() {
    ...
    way=$4
    set -x
    resp=$(curl -ksd '{"method":"bnchain.CreateTransaction","params":[{"execer":"lottery","actionName":"LotteryBuy",
...
lottery_LotteryDraw() {
    ...
    priv=$1
    set -x
    resp=$(curl -ksd '{"method":"bnchain.CreateTransaction","params":[{"execer":"lottery","actionName":"LotteryDraw",
...
lottery_LotteryClose() {
    ...
    priv=$1
    set -x
    resp=$(curl -ksd '{"method":"bnchain.CreateTransaction","params":[{"execer":"lottery","actionName":"LotteryClose",
    "payload":{"lotteryId":"'"$gID"'","fee":1000000}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
    ok=$(echo "${resp}" | jq -r ".error")
    [[ $ok == null ]]
    rst=$?
...
lottery_GetLotteryNormalInfo() {
    ...
    set -x
    resp=$(curl -ksd '{"method":"bnchain.Query","params":[{"execer":"'"$execer"'","funcName":"'"$funcName"'","payload":{"lotteryId":"'"$gameID"'"}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
...
lottery_GetLotteryRoundLuckyNumber() {
    ...
    set -x
    resp=$(curl -ksd '{"method":"bnchain.Query","params":[{"execer":"'"$execer"'","funcName":"'"$funcName"'","payload":{"lotteryId":"'"$gameID"'", "round":['"$round"']}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
...
lottery_GetLotteryHistoryBuyInfo() {
    ...
    set -x
    resp=$(curl -ksd '{"method":"bnchain.Query","params":[{"execer":"'"$execer"'","funcName":"'"$funcName"'","payload":{"lotteryId":"'"$gameID"'", "addr":"'"$addr"'"}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
...
lottery_GetLotteryBuyRoundInfo() {
    ...
    set -x
    resp=$(curl -ksd '{"method":"bnchain.Query","params":[{"execer":"'"$execer"'","funcName":"'"$funcName"'","payload":{"lotteryId":"'"$gameID"'", "addr":"'"$addr"'", "round":'"$round"'}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
    ok=$(jq '(.error|not) and (.result.records | length == '"$count"') and (.result.records[0].number == "'"$number"'")' <<<"$resp")
    set +x
    [[ $ok == true ]]
    rst=$?
...
function run_testcases() {
    ...
    set -x
    #给游戏合约中转帐
    bnchain_SendToAddress "${gameAddr1}" "${lottExecAddr}" 500000000 "${MAIN_HTTP}"
    bnchain_SendToAddress "${gameAddr2}" "${lottExecAddr}" 500000000 "${MAIN_HTTP}"
    set +x
    ...
    bnchain_BlockWait ${drawNum} "${M_HTTP}"

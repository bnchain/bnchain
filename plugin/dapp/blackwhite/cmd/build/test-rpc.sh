...
init() {
    ...
    if [ "$ispara" == true ]; then
        bwExecAddr=$(curl -ksd '{"method":"bnchain.ConvertExectoAddr","params":[{"execname":"user.p.para.blackwhite"}]}' ${MAIN_HTTP} | jq -r ".result")
    else
        bwExecAddr=$(curl -ksd '{"method":"bnchain.ConvertExectoAddr","params":[{"execname":"blackwhite"}]}' ${MAIN_HTTP} | jq -r ".result")
    fi
...
bnchain_NewAccount() {
    label=$1
    result=$(curl -ksd '{"jsonrpc":"2.0","id":2,"method":"bnchain.NewAccount","params":[{"label":"'"$label"'"}]}' -H 'content-type:text/plain;' ${MAIN_HTTP} | jq -r ".result.acc.addr")
    [ "$result" != "" ]
    rst=$?
...
bnchain_SendTransaction() {
    ...
    addr=$2
    #签名交易
    resp=$(curl -ksd '{"jsonrpc":"2.0","id":2,"method":"bnchain.SignRawTx","params":[{"addr":"'"$addr"'","txHex":"'"$rawTx"'","expire":"120s","fee":10000000,"index":0}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
    ...
    resp=$(curl -ksd '{"jsonrpc":"2.0","id":2,"method":"bnchain.SendTransaction","params":[{"data":"'"$signTx"'"}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
    ok=$(echo "${resp}" | jq -r ".error")
    [ "$ok" == null ]
    rst=$?
    echo_rst "$FUNCNAME" "$rst"
    ...
    bnchain_QueryTx "$gResp" "${MAIN_HTTP}"
}
...
blackwhite_BlackwhiteCreateTx() {
    ...
    bnchain_SendTransaction "${rawTx}" "${addr}"
    gID="${gResp}"
    echo "gameID $gID"
}
...
blackwhite_GetBlackwhiteRoundInfo() {
    ...
    resp=$(curl -ksd '{"jsonrpc":"2.0","id":2,"method":"bnchain.Query","params":[{"execer":"'"$execer"'","funcName":"'"$funcName"'","payload":{"gameID":"'"$gameID"'"}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
...
blackwhite_GetBlackwhiteByStatusAndAddr() {
    ...
    resp=$(curl -ksd '{"jsonrpc":"2.0","id":2,"method":"bnchain.Query","params":[{"execer":"'"$execer"'","funcName":"'"$funcName"'","payload":{"status":5,"address":"'"$addr"'","count":1,"direction":0,"index":-1}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
...
blackwhite_GetBlackwhiteloopResult() {
    ...
    resp=$(curl -ksd '{"jsonrpc":"2.0","id":2,"method":"bnchain.Query","params":[{"execer":"'"$execer"'","funcName":"'"$funcName"'","payload":{"gameID":"'"$gameID"'","loopSeq":0}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
...
function run_testcases() {
    ...
    #先创建账户地址
    bnchain_NewAccount "label188"
    gameAddr1="${glAddr}"
    bnchain_NewAccount "label288"
    gameAddr2="${glAddr}"
    bnchain_NewAccount "label388"
    gameAddr3="${glAddr}"

    #给每个账户分别转帐
    origAddr="12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv"

    bnchain_GetAccounts "${MAIN_HTTP}"

    #主链中相应账户需要转帐
    M_HTTP=${MAIN_HTTP//8901/8801}
    bnchain_SendToAddress "${origAddr}" "${gameAddr1}" 1000000000 "${M_HTTP}"
    bnchain_SendToAddress "${origAddr}" "${gameAddr2}" 1000000000 "${M_HTTP}"
    bnchain_SendToAddress "${origAddr}" "${gameAddr3}" 1000000000 "${M_HTTP}"

    #平行链相应账户需要转帐
    bnchain_SendToAddress "${origAddr}" "${gameAddr1}" 1000000000 "${MAIN_HTTP}"
    bnchain_SendToAddress "${origAddr}" "${gameAddr2}" 1000000000 "${MAIN_HTTP}"
    bnchain_SendToAddress "${origAddr}" "${gameAddr3}" 1000000000 "${MAIN_HTTP}"

    #给游戏合约中转帐
    bnchain_SendToAddress "${gameAddr1}" "${bwExecAddr}" 500000000 "${MAIN_HTTP}"
    bnchain_SendToAddress "${gameAddr2}" "${bwExecAddr}" 500000000 "${MAIN_HTTP}"
    bnchain_SendToAddress "${gameAddr3}" "${bwExecAddr}" 500000000 "${MAIN_HTTP}"

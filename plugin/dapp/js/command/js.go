...
import (
	...
	jsty "github.com/bnchain/plugin/plugin/dapp/js/types"
...
func createJavaScriptContract(cmd *cobra.Command, args []string) {
	...
	var res string
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.CreateTransaction", params, &res)
...
func queryJavaScript(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "bnchain.Query", params, rep)
	ctx.Run()
}

...
function init() {
    ...
    exec_addr=$(curl -ksd '{"method":"bnchain.ConvertExectoAddr","params":[{"execname":"'${exec_name}'"}]}' ${MAIN_HTTP} | jq -r ".result")
...
function configJSCreator() {
    req='{"jsonrpc": "2.0", "method" :  "bnchain.CreateTransaction" , "params":[{"execer":"'${manager_name}'","actionName":"Modify","payload":{"key":"js-creator","op":"add", "value" : "'${beneficiary}'"}}]}'
    echo "#request: $req"
    ...
    bnchain_SignRawTx "$rawtx" "${super_manager}" "${MAIN_HTTP}"
}

function createJSContract() {
    req='{"jsonrpc": "2.0", "method" :  "bnchain.CreateTransaction" , "params":[{"execer":"'${exec_name}'","actionName":"Create","payload":{"name":"'${game}'","code":"'${jsCode}'"}}]}'
    echo "#request: $req"
    resp=$(curl -ksd "$req" "${MAIN_HTTP}")
    # echo "#resp: $resp"
    ok=$(jq '(.error|not) and (.result != "")' <<<"$resp")
    ...
    echo_rst "$FUNCNAME" "$?"
    rawtx=$(jq -r ".result" <<<"$resp")
    bnchain_SignRawTx "$rawtx" "${beneficiary_key}" "${MAIN_HTTP}"
}

function callJS() {
    #the_exec=
    req='{"jsonrpc": "2.0", "method" :  "bnchain.CreateTransaction" , "params":[{"execer":"'${user_game}'","actionName":"Call","payload":{"name":"'${game}'","funcname":"hello", "args" : "{}"}}]}'
    # echo "#request: $req"
    resp=$(curl -ksd "$req" "${MAIN_HTTP}")
    # echo "#resp: $resp"
    ok=$(jq '(.error|not) and (.result != "")' <<<"$resp")
...
function queryJS() {
    req='{"jsonrpc": "2.0", "method" :  "bnchain.Query" , "params":[{"execer":"'${user_game}'","funcName":"Query","payload":{"name":"'${game}'","funcname":"hello", "args" : "{}"}}]}'
    # echo "#request: $req"
    resp=$(curl -ksd "$req" "${MAIN_HTTP}")
    # echo "#resp: $resp"
    ok=$(jq '(.error|not) and (.result != "")' <<<"$resp")

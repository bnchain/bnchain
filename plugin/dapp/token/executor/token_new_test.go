...
import (
	...
	pty "github.com/bnchain/plugin/plugin/dapp/token/types"
...
var (
	...
	mainClient      types.bnchainClient
	paraClient      types.bnchainClient
	r               *rand.Rand
...
func init() {
	...
	mainClient = types.NewbnchainClient(conn)

	conn, err = grpc.Dial(ParaNetgrpcAddr, grpc.WithInsecure())
	if err != nil {
		panic(err)
	}
	paraClient = types.NewbnchainClient(conn)
...
func TestToken_validSymbolWithHeight(t *testing.T) {
	types.SetTitleOnlyForTest("bnchain")

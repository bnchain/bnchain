#!/usr/bin/env bash
PARA_CLI="docker exec ${NODE3} /root/bnchain-para-cli"

PARA_CLI2="docker exec ${NODE2} /root/bnchain-para-cli"
PARA_CLI1="docker exec ${NODE1} /root/bnchain-para-cli"
PARA_CLI4="docker exec ${NODE4} /root/bnchain-para-cli"
MAIN_CLI="docker exec ${NODE3} /root/bnchain-cli"
...
function para_init() {
    para_set_toml bnchain.para33.toml
    para_set_toml bnchain.para32.toml
    para_set_toml bnchain.para31.toml
    para_set_toml bnchain.para30.toml

    sed -i $xsedfix 's/^authAccount=.*/authAccount="1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4"/g' bnchain.para33.toml
    sed -i $xsedfix 's/^authAccount=.*/authAccount="1JRNjdEqp4LJ5fqycUBm9ayCKSeeskgMKR"/g' bnchain.para32.toml
    sed -i $xsedfix 's/^authAccount=.*/authAccount="1NLHPEcbTWWxxU3dGUZBhayjrCHD3psX7k"/g' bnchain.para31.toml
    sed -i $xsedfix 's/^authAccount=.*/authAccount="1MCftFynyvG2F4ED5mdHYgziDxx6vDrScs"/g' bnchain.para30.toml
}

function para_set_toml() {
    cp bnchain.para.toml "${1}"
...
function token_transfer() {
    ...
    # $ ./build/bnchain-cli   exec addr  -e user.p.para.paracross

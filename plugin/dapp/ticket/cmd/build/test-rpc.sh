...
ticket_CreateBindMiner() {
    ...
    bnchain_SignRawTx "${rawTx}" "${returnPriv}" ${MAIN_HTTP}
    set +x
}
...
ticket_TicketInfos() {
    ...
    set -x
    resp=$(curl -ksd '{"method":"bnchain.Query","params":[{"execer":"'"$execer"'","funcName":"'"$funcName"'","payload":{"ticketIds":["'"$tid"'"]}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
...
ticket_TicketList() {
    ...
    execer="ticket"
    funcName="TicketList"
    resp=$(curl -ksd '{"method":"bnchain.Query","params":[{"execer":"'"$execer"'","funcName":"'"$funcName"'","payload":{"addr":"'"$minerAddr"'", "status":'"$status"'}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
    set -x
    ok=$(jq '(.error|not) and (.result.tickets | length > 0) and (.result.tickets[0].minerAddress == "'"$minerAddr"'") and (.result.tickets[0].returnAddress == "'"$returnAddr"'") and (.result.tickets[0].status == '"$status"')' <<<"$resp")
    set +x
...
ticket_MinerAddress() {
    ...
    set -x
    resp=$(curl -ksd '{"method":"bnchain.Query","params":[{"execer":"'"$execer"'","funcName":"'"$funcName"'","payload":{"data":"'"$returnAddr"'"}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
...
ticket_MinerSourceList() {
    ...
    set -x
    resp=$(curl -ksd '{"method":"bnchain.Query","params":[{"execer":"'"$execer"'","funcName":"'"$funcName"'","payload":{"data":"'"$minerAddr"'"}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
...
ticket_RandNumHash() {
    ...
    set -x
    resp=$(curl -ksd '{"method":"bnchain.Query","params":[{"execer":"'"$execer"'","funcName":"'"$funcName"'","payload":{"hash":"'"$hash"'", "blockNum":'"$blockNum"'}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
...
function run_testcases() {
    ...
    set -x
    bnchain_SendToAddress "${minerAddr1}" "${minerAddr2}" 100000000 "${MAIN_HTTP}"
    bnchain_SendToAddress "${minerAddr1}" "${returnAddr2}" $((price + 5 * 100000000)) "${MAIN_HTTP}"
    set +x
    ...
    bnchain_LastBlockhash "${MAIN_HTTP}"

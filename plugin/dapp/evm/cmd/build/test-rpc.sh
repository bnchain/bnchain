...
function bnchain_ImportPrivkey() {
    local pri=$2
    local acc=$3
    local req='"method":"bnchain.ImportPrivkey", "params":[{"privkey":"'"$pri"'", "label":"admin"}]'
    echo "#request: $req"
    resp=$(curl -ksd "{$req}" "$1")
    echo "#response: $resp"
...
function evm_addressCheck() {
    res=$(curl -s --data-binary '{"jsonrpc":"2.0","id":2,"method":"bnchain.Query","params":[{"execer":"evm","funcName":"CheckAddrExists","payload":{"addr":"'${evm_contractAddr}'"}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
...
function evm_abiGet() {
    abiInfo=$(curl -s --data-binary '{"jsonrpc":"2.0","id":2,"method":"bnchain.Query","params":[{"execer":"evm","funcName":"QueryABI","payload":{"address":"'${evm_contractAddr}'"}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
...
function evm_getBalance() {
    ...
    res=$(curl -s --data-binary '{"jsonrpc":"2.0","id":2,"method":"bnchain.GetBalance","params":[{"addresses":["'${evm_creatorAddr}'"],"execer":"'${evm_addr}'", "paraName": "'${paraName}'"}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
...
function queryTransaction() {
    ...
    res=$(curl -s --data-binary '{"jsonrpc":"2.0","id":2,"method":"bnchain.QueryTransaction","params":[{"hash":"'"${txHash}"'"}]}' -H 'content-type:text/plain;' ${MAIN_HTTP})
...
function init() {
    ...
    local evm_addr=""
    if [ "$ispara" == "true" ]; then
        evm_addr=$(curl -ksd '{"method":"bnchain.ConvertExectoAddr","params":[{"execname":"user.p.para.evm"}]}' ${MAIN_HTTP} | jq -r ".result")
        bnchain_SendToAddress "$from" "$evm_addr" 10000000000
    else
        evm_addr=$(curl -ksd '{"method":"bnchain.ConvertExectoAddr","params":[{"execname":"evm"}]}' ${MAIN_HTTP} | jq -r ".result")
    fi
    echo "evm=$evm_addr"

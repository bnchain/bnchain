...
import (
	...
	rt "github.com/bnchain/plugin/plugin/dapp/retrieve/types"
...
func checkexecAcc(c types.bnchainClient, addr string, sorc int, balance int64) bool {
	var addrs []string
...
func showOrCheckAcc(c types.bnchainClient, addr string, sorc int, balance int64) bool {
	req := &types.ReqNil{}
...
func sendtoaddress(c types.bnchainClient, priv crypto.PrivKey, to string, amount int64) ([]byte, error) {
	//defer conn.Close()

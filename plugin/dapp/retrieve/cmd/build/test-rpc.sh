...
retrieve_Backup() {
    ...
    local reqDecode='"method":"bnchain.DecodeRawTransaction","params":[{"txHex":"'"$tx"'"}]'
...
retrieve_QueryResult() {
    ...
    local req='"method":"bnchain.Query","params":[{"execer":"retrieve","funcName":"GetRetrieveInfo","payload":{"backupAddress":"1EDnnePAZN48aC2hiTDzhkczfF39g1pZZX", "defaultAddress":"1PUiGcbsccfxW3zuvHXZBJfznziph5miAo"}}]'
...
init() {
    ...
    echo "ipara=$ispara"
    if [ "$ispara" == true ]; then
        retrieve_addr=$(curl -ksd '{"method":"bnchain.ConvertExectoAddr","params":[{"execname":"user.p.para.retrieve"}]}' ${MAIN_HTTP} | jq -r ".result")
    else
        retrieve_addr=$(curl -ksd '{"method":"bnchain.ConvertExectoAddr","params":[{"execname":"retrieve"}]}' ${MAIN_HTTP} | jq -r ".result")
    fi

    local from="1PUiGcbsccfxW3zuvHXZBJfznziph5miAo"
    bnchain_SendToAddress "$from" "$retrieve_addr" 1000000000 ${MAIN_HTTP}

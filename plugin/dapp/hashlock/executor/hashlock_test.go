...
import (
	...
	hlt "github.com/bnchain/plugin/plugin/dapp/hashlock/types"
...
func showAccount(c types.bnchainClient, addr string) {
	req := &types.ReqNil{}
	accs, err := c.GetAccounts(context.Background(), req)
	if err != nil {
		panic(err)
...
func sendtoaddress(c types.bnchainClient, priv crypto.PrivKey, to string, amount int64) error {
	//defer conn.Close()

...
import (
	...
	jsonrpc "github.com/bnchain/bnchain/rpc/jsonclient"
	rpctypes "github.com/bnchain/bnchain/rpc/types"
	"github.com/bnchain/bnchain/types"
	pkt "github.com/bnchain/plugin/plugin/dapp/pokerbull/types"
...
func pokerbullStart(cmd *cobra.Command, args []string) {
	...
	var res string
	ctx := jsonrpc.NewRPCCtx(rpcLaddr, "bnchain.CreateTransaction", params, &res)
...
func pokerbullQuery(cmd *cobra.Command, args []string) {
	...
	if gameID != "" {
		if round == "" {
			...
			ctx := jsonrpc.NewRPCCtx(rpcLaddr, "bnchain.Query", params, &res)
			ctx.Run()
		} else {

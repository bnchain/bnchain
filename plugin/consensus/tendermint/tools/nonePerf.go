...
import (
	...
	ty "github.com/bnchain/plugin/plugin/dapp/valnode/types"
)
...
func Put(ip string, size string, privkey string) {
	...
	poststr := fmt.Sprintf(`{"jsonrpc":"2.0","id":2,"method":"bnchain.SendTransaction","params":[{"data":"%v"}]}`,
...
func Get(ip string, hash string) {
	...
	poststr := fmt.Sprintf(`{"jsonrpc":"2.0","id":2,"method":"bnchain.QueryTransaction","params":[{"hash":"%s"}]}`, hash)
	resp, err := http.Post(url, "application/json", bytes.NewBufferString(poststr))
	if err != nil {
		fmt.Println(err)
		return
...
func setTxHeight(ip string) {
	url := "http://" + ip + ":8801"
	poststr := fmt.Sprintf(`{"jsonrpc":"2.0","id":2,"method":"bnchain.GetLastHeader","params":[]}`)
	resp, err := http.Post(url, "application/json", bytes.NewBufferString(poststr))
	if err != nil {
		fmt.Println(err)
		return
